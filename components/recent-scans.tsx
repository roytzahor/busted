...
export function RecentScans() {
  ...
  // Portal target only exists after mount (client). The drawer + backdrop must
  // render into document.body — the nav <header> uses backdrop-blur, which
  // establishes a containing block for fixed-positioned descendants, so a
  // drawer rendered inside it would collapse to the header's height instead of
  // filling the viewport. Portalling to body escapes that containing block.
                      ...
                      {entry.scanId ? (
                        ...
                        <a
                          ...
                          onClick={(e) => {
                            ...
                            setOpen(false);
                            track("history_click", {
                              props: { url: entry.url, target: "permalink" },
                            });
                          }}
